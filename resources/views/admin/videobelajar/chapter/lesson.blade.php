...
@section('title')
    <title>Admin Digidik - {{$chapter->nama}}</title>
@endsection
...
@section('heading')
    ...
    <div class="page-title d-flex flex-column align-items-start justify-content-center flex-wrap me-lg-2 pb-2 pb-lg-0" data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', lg: '#kt_header_container'}">
        <!--begin::Heading-->
        <h1 class="fs-1 fw-bolder text-dark">Video Pada {{$chapter->nama}}</h1>
        <!--end::Heading-->
        <!--begin::Breadcrumb-->
        <ul class="breadcrumb fw-bold fs-base my-1">
            ...
            <li class="breadcrumb-item text-muted">
                <a href="{{route('admin.video.subject.chapter',$chapter->subject->id)}}" class="text-muted">{{$chapter->subject->nama}}</a>
            </li>
            <li class="breadcrumb-item">{{$chapter->nama}}</li>
        </ul>
        <!--end::Breadcrumb-->
    </div>
    <!--end::Page title=-->
...
<div class="card card-custom card-stretch gutter-b mt-6">
    ...
    <div class="card-header border-0 pt-5">
        <h3 class="card-title align-items-start flex-column">
            <span class="card-label font-weight-bolder text-dark">Berikut Video Digidik</span>
            <span class="text-muted mt-3 font-weight-bold font-size-sm">Video merupakan entitas terkecil dalam fitur video belajar.</span>
        </h3>
                            ...
                            @foreach($video as $index=>$data)
                                <tr>
                                    ...
                                    <td class="pl-0">
                                        ...
                                        <span class="text-muted font-weight-bold d-block">Slug: {{$data->slug}}, Mapel: {{$data->chapter->nama}}</span>
                                    </td>
                                    <td class="text-right">
                                        <span class="text-muted font-weight-bold">{{$data->pengajar}}</span>
                                    </td>
                                    <td class="text-right">
                                    ...
                                    <td class="text-right pr-0">
                                        ...
                                        <a target="_blank" href="{{route('video.lesson',[$data->chapter->subject->group->level->slug,$data->chapter->subject->group->slug,$data->chapter->subject->slug,$data->chapter->slug,$data->slug])}}" class="btn btn-icon btn-light btn-sm">
