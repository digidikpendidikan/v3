...
@section('title')
    <title>Admin Digidik - {{$subject->nama}}</title>
@endsection
...
@section('heading')
    ...
    <div class="page-title d-flex flex-column align-items-start justify-content-center flex-wrap me-lg-2 pb-2 pb-lg-0" data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', lg: '#kt_header_container'}">
        <!--begin::Heading-->
        <h1 class="fs-1 fw-bolder text-dark">Bab Pada {{$subject->nama}}</h1>
        <!--end::Heading-->
        <!--begin::Breadcrumb-->
        <ul class="breadcrumb fw-bold fs-base my-1">
            ...
            <li class="breadcrumb-item">{{$subject->nama}}</li>
        </ul>
        <!--end::Breadcrumb-->
    </div>
    <!--end::Page title=-->
...
<div class="card card-custom card-stretch gutter-b mt-6">
    ...
    <div class="card-header border-0 pt-5">
        <h3 class="card-title align-items-start flex-column">
            <span class="card-label font-weight-bolder text-dark">Berikut Bab Video Digidik</span>
            <span class="text-muted mt-3 font-weight-bold font-size-sm">Masing-masing bab memiliki beberapa video.</span>
        </h3>
                            ...
                            @foreach($chapter as $index=>$data)
                                <tr>
                                    ...
                                    <td class="pl-0">
                                        ...
                                        <span class="text-muted font-weight-bold d-block">Slug: {{$data->slug}}, Mapel: {{$data->subject->nama}}</span>
                                    </td>
                                    <td class="text-right">
                                    ...
                                    <td class="text-right pr-0">
                                        ...
                                        <a href="{{route('admin.video.chapter.lesson',$data->id)}}" class="btn btn-icon btn-light btn-sm">
                                            <span class="svg-icon svg-icon-md svg-icon-success">
                                                Video
                                            </span>
                                        </a>
                                        <a target="_blank" href="{{route('video.subject',[$data->subject->group->level->slug,$data->subject->group->slug,$data->subject->slug])}}" class="btn btn-icon btn-light btn-sm">
                                            <span class="svg-icon svg-icon-md svg-icon-success">
